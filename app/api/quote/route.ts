...
export async function POST(request: Request) {
  try {
    ...
    for (let i = 0; i < validatedItems.length; i++) {
      ...
      console.log(`[quote:debug]   Layer height        : ${settings.layerHeightMm} mm  |  Infill: ${settings.infillPercent}%  |  Supports: ${settings.removeSupports}`);
      console.log(`[quote:debug]   ── Volume breakdown (per unit) ──`);
      console.log(`[quote:debug]   Shell volume        : ${q.shellVolumeCm3.toFixed(3)} cm³  (SA ${surfaceAreaMm2.toFixed(0)} mm² × 0.8 mm / 1000)`);
      console.log(`[quote:debug]   Infill volume       : ${q.infillVolumeCm3.toFixed(3)} cm³  ((solid − shell) × ${settings.infillPercent}%)`);
      console.log(`[quote:debug]   Printed volume      : ${q.printedVolumeCm3.toFixed(3)} cm³  (shell+infill${settings.removeSupports ? " × 1.20 supports" : ""})`);
      console.log(`[quote:debug]   Weight/unit         : ${q.estimatedWeightGrams.toFixed(2)} g`);
      console.log(`[quote:debug]   ── Time breakdown (per unit) ──`);
      console.log(`[quote:debug]   Print time/unit     : ${q.estimatedPrintTimeMinutes} min`);
      console.log(`[quote:debug]     startup           : 3 min  |  layer overhead: ${heightMm > 0 ? Math.ceil(heightMm / settings.layerHeightMm) : "?"} layers × 3 s`);
      console.log(`[quote:debug]     shell flow        : 11 mm³/s × ${(settings.layerHeightMm / 0.2).toFixed(2)} scale`);
      console.log(`[quote:debug]     infill flow       : 20 mm³/s × ${(settings.layerHeightMm / 0.2).toFixed(2)} scale`);
